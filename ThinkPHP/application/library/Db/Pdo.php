...
class Db_Pdo extends Db{
    ...
    public function connect($config='',$linkNum=0) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            //$config['params'][PDO::ATTR_CASE] = Think::C("DB_CASE_LOWER")?PDO::CASE_LOWER:PDO::CASE_UPPER;
            try{
                $this->linkID[$linkNum] = new \PDO( $config['dsn'], $config['username'], $config['password'],$config['params']);
            }catch (\PDOException $e) {
                Think::E($e->getMessage());
            }
            ...
            if(in_array($this->dbType,array('MSSQL','ORACLE','IBASE','OCI'))) {
                ...
                Think::E('由于目前PDO暂时不能完美支持'.$this->dbType.' 请使用官方的'.$this->dbType.'驱动');
            }
            $this->linkID[$linkNum]->exec('SET NAMES '.$config['charset']);
            // 标记连接成功
            $this->connected    =   true;
            // 注销数据库连接配置信息
            if(1 != Think::C('DB_DEPLOY_TYPE')) unset($this->config);
        }
    ...
    public function execute($str,$bind=array()) {
        ...
        if($this->dbType == 'OCI') {
            if(preg_match("/^\s*(INSERT\s+INTO)\s+(\w+)\s+/i", $this->queryStr, $match)) {
                $this->table = Think::C("DB_SEQUENCE_PREFIX").str_ireplace(Think::C("DB_PREFIX"), "", $match[2]);
    ...
    public function getFields($tableName) {
        $this->initConnect(true);
        if(Think::C('DB_DESCRIBE_TABLE_SQL')) {
            // 定义特殊的字段查询SQL
            $sql   = str_replace('%table%',$tableName,Think::C('DB_DESCRIBE_TABLE_SQL'));
        }else{
            switch($this->dbType) {
                case 'MSSQL':
                case 'SQLSRV':
    ...
    public function getTables($dbName='') {
        if(Think::C('DB_FETCH_TABLES_SQL')) {
            // 定义特殊的表查询SQL
            $sql   = str_replace('%db%',$dbName,Think::C('DB_FETCH_TABLES_SQL'));
        }else{
            switch($this->dbType) {
            case 'ORACLE':
            case 'OCI':
            ...
            case 'IBASE':
                // 暂时不支持
                Think::E(Think::L('_NOT_SUPPORT_DB_').':IBASE');
                break;
            case 'SQLITE':
