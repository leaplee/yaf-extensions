...
class Db {
    ...
    public static function getInstance($db_config='') {
		...
		$guid	=	Think::to_guid_string($db_config);
    ...
    public function factory($db_config='') {
        ...
        if(empty($db_config['dbms']))
            Think::E(Think::L('_NO_DB_CONFIG_'));
        // 数据库类型
        ...
        if(class_exists($class)) {
            ...
            // 类没有定义
            Think::E(Think::L('_NO_DB_DRIVER_').': ' . $class);
        }
        return $db;
    }
    ...
    private function parseConfig($db_config='') {
        if ( !empty($db_config) && is_string($db_config)) {
            ...
            // 如果配置为空，读取配置文件设置
            if( Think::C('DB_DSN') && 'pdo' != strtolower(Think::C('DB_TYPE')) ) { // 如果设置了DB_DSN 则优先
                $db_config =  $this->parseDSN(Think::C('DB_DSN'));
            }else{
                $db_config = array (
                    'dbms'      =>  Think::C('DB_TYPE'),
                    'username'  =>  Think::C('DB_USER'),
                    'password'  =>  Think::C('DB_PWD'),
                    'hostname'  =>  Think::C('DB_HOST'),
                    'hostport'  =>  Think::C('DB_PORT'),
                    'database'  =>  Think::C('DB_NAME'),
                    'dsn'       =>  Think::C('DB_DSN'),
                    'params'    =>  Think::C('DB_PARAMS'),
                    'charset'   =>  Think::C('DB_CHARSET'),
                );
            }
        }
        return $db_config;
    ...
    protected function multiConnect($master=false) {
        ...
        // 数据库读写是否分离
        if(Think::C('DB_RW_SEPARATE')){
            // 主从式采用读写分离
            if($master)
                // 主服务器写入
                $r  =   floor(mt_rand(0,Think::C('DB_MASTER_NUM')-1));
            else{
                if(is_numeric(Think::C('DB_SLAVE_NO'))) {// 指定服务器读
                    $r = Think::C('DB_SLAVE_NO');
                }else{
                    // 读操作连接从服务器
                    $r = floor(mt_rand(Think::C('DB_MASTER_NUM'),count($_config['hostname'])-1));   // 每次随机连接的数据库
                }
            }
        }else{
            // 读写操作不区分服务器
    ...
    protected function parseSet($data) {
        foreach ($data as $key=>$val){
            if(is_array($val) && 'exp' == $val[0]){
              ...
              if(Think::C('DB_BIND_PARAM') && 0 !== strpos($val,':')){
                $name   =   md5($key);
            ...
            foreach ($where as $key=>$val){
                ...
                if(0===strpos($key,'_')) {
                    ...
                    if(!preg_match('/^[A-Z_\|\&\-.a-z0-9\(\)\,]+$/',trim($key))){
                        Think::E(Think::L('_EXPRESS_ERROR_').':'.$key);
                    }
                    // 多条件支持
        ...
        if(is_array($val)) {
            if(is_string($val[0])) {
                if(preg_match('/^(EQ|NEQ|GT|EGT|LT|ELT)$/i',$val[0])) { // 比较运算
                    ...
                    Think::E(Think::L('_EXPRESS_ERROR_').':'.$val[0]);
                }
            }else {
                $count = count($val);
            ...
            //对字符串类型字段采用模糊匹配
            if(Think::C('DB_LIKE_FIELDS') && preg_match('/('.Think::C('DB_LIKE_FIELDS').')/i',$key)) {
                $val  =  '%'.$val.'%';
    ...
    public function buildSelectSql($options=array()) {
        ...
        if(Think::C('DB_SQL_BUILD_CACHE')) { // SQL创建缓存
        ...
        if(isset($key)) { // 写入SQL创建缓存
            Think::S($key,$sql,array('expire'=>0,'length'=>Think::C('DB_SQL_BUILD_LENGTH'),'queue'=>Think::C('DB_SQL_BUILD_QUEUE')));
        }
        return $sql;
    }
