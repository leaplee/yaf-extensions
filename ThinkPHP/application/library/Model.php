...
class Model {
    ...
    protected function _checkTableInfo() {
        ...
        if(empty($this->fields)) {
            // 如果数据表字段没有定义则自动获取
            if(Think::C('DB_FIELDS_CACHE')) {
                $db   =  $this->dbName?$this->dbName:Think::C('DB_NAME');
                $fields = Think::F('_fields/'.strtolower($db.'.'.$this->name));
                if($fields) {
                    $this->fields   =   $fields;
                    $this->pk       =   $fields['_pk'];
                    return ;
    ...
    public function flush() {
        ...
        if(Think::C('DB_FIELDS_CACHE')){
            // 永久缓存数据表信息
            $db   =  $this->dbName?$this->dbName:Think::C('DB_NAME');
            Think::F('_fields/'.strtolower($db.'.'.$this->name),$this->fields);
        }
    }
    ...
    public function add($data='',$options=array(),$replace=false) {
        if(empty($data)) {
            ...
            if(!empty($this->data)) {
                ...
                $this->error    = Think::L('_DATA_TYPE_INVALID_');
                return false;
            }
        }
        // 分析表达式
    ...
    public function selectAdd($fields='',$table='',$options=array()) {
        ...
        if(false === $result = $this->db->selectInsert($fields?$fields:$options['field'],$table?$table:$this->getTableName(),$options)){
            // 数据库插入操作失败
            $this->error = Think::L('_OPERATION_WRONG_');
            return false;
        }else {
            // 插入成功
            return $result;
    ...
    protected function _read_data($data) {
        // 检查字段映射
        if(!empty($this->_map) && Think::C('READ_DATA_MAP')) {
     ...
     */
     public function create($data='',$type='') {
        ...
        if(isset($fields)) {
            ...
            // 判断令牌验证字段
            if(Think::C('TOKEN_ON'))   $fields[] = Think::C('TOKEN_NAME');
            foreach ($data as $key=>$val){
        ...
        if(!$this->autoCheckToken($data)) {
            $this->error = Think::L('_TOKEN_ERROR_');
            return false;
        }
    ...
    public function autoCheckToken($data) {
        ...
        if(Think::C('TOKEN_ON')){
            ...
            // 开启TOKEN重置
            if(Think::C('TOKEN_RESET')) unset($_SESSION[$name][$key]);
            return false;
        }
        return true;
    }
            ...
            foreach($_validate as $key=>$val) {
                ...
                if(empty($val[5]) || $val[5]== self::MODEL_BOTH || $val[5]== $type ) {
                    if(0==strpos($val[2],'{%') && strpos($val[2],'}'))
                        ...
                        $val[2]  =  Think::L(substr($val[2],2,-1));
    ...
    protected function parseSql($sql,$parse) {
        ...
        if(true === $parse) {
            ...
            $sql    =   strtr($sql,array('__TABLE__'=>$this->getTableName(),'__PREFIX__'=>Think::C('DB_PREFIX')));
        }
        $this->db->setModel($this->name);
        return $sql;
    }
    ...
    public function data($data=''){
        ...
        if(is_object($data)){
            ...
            Think::E(Think::L('_DATA_TYPE_INVALID_'));
        }
        $this->data = $data;
        return $this;
    }
